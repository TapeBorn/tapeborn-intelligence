...
| GAP-C | Raising `allocationCap` on an `EXHAUSTED` phase does not reopen it (`EXHAUSTED → ACTIVE` rejected). An exhausted phase is permanent; a raised cap is inert. | Medium (ops/UX) | Documented + asserted (group 4). **Needs founder decision** — contract change required if reopening is desired (outside BUILD_028B). |
| GAP-D | **SPEC DEVIATION (corrected 2026-09-26 — the earlier "matches Section 21" note was wrong on the event NAME).** Lock SECTION 21 line 501 requires the event to be named `ClaimCampaignPhase(bytes32,bytes32,address,uint256)`; the contract emits `Claim(...)` (`TapeBornProductionNFT.sol:63`) and the tests assert `"Claim"`. Arguments/order/types **do** match, and field 4 is `tokenId` (not a quantity). §21 line 509 forbids "or similar". | Medium-High (integration / spec conformance) | ⛔ **C. UNRESOLVED** — rename event + 2 test asserts (free before deploy, breaking after), or founder amends §21. See `BUILD_028B_FINAL_CLOSURE_REVIEW.md` PHASE 4. |
