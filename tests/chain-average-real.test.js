...
// Tests for real rolling chain average volume calculation
...
test('Real chain average: first block records volume', () => {
  ...
  // Update average - should compute from block 100
  const avg = state.updateChainAverageVolume(5042002, 100, 10**6);
...
test('Real chain average: multiple blocks compute average', () => {
  cleanupTestDb();
  resetSignalState();
  const state = getSignalState(TEST_DB);
  ...
  // Average = (100+200+300+400+500+600) / 6 = 2100 / 6 = 350
  const avg = state.updateChainAverageVolume(5042002, 105, 10**6);
  assert.strictEqual(avg, 350);
...
test('Real chain average: duplicate block processing', () => {
  ...
  const avg = state.updateChainAverageVolume(5042002, 100, 10**6);
  // Should use the latest value (200)
...
test('Real chain average: decimal conversion precision', () => {
  ...
  // Average = (333.33 + 666.67) / 2 = 500
  assert.ok(Math.abs(avg - 500) < 0.01);
